...
function Editor_() {
  ...
  const [thinkingIndex, setThinkingState] = useState(0);
  ...
  const handleUpdate = async () => {
    ...
    const promptToBeSend = prompt;
    setPrompt('');
    setMessages((m) => [...m, { role: 'user', content: promptToBeSend }]);
    try {
      const result = await axios.post(
        `${serverUrl}/api/website/update/${id}`,
        { prompt: promptToBeSend },
        { withCredentials: true },
      );
      ...
      setUpdateLoading(false);
      setMessages((m) => [...m, { role: 'ai', content: 'some error occured' }]);
  ...
  useEffect(() => {
    if (updateLoading) {
      const id = setInterval(() => {
        setThinkingState((i) => (i + 1) % thinkingSteps.length);
      }, 1200);
  ...
  if (error) {
    // navigate('/dashboard');
    return (
      <div className="w-screen h-screen bg-black flex flex-col items-center justify-center gap-2">
  ...
  return (
    <>
      <div className="h-10 bg-black/94 px-4 flex items-center gap-2 border-b border-white/10">
        <button
